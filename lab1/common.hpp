...
struct message {
    ...
    int set(msg_type tp,const char*msg,int sz){
        if(sz > MSG_LEN)return -1;
        length=sz;
        type = tp;
        for(int i = 0;i < sz;i++){
            message[i] = msg[i];
    ...
    int printToBuf(char *buf, int sz) const{
        if (length > MSG_LEN) return -2; // invalid length
    ...
    int parseFromBuf(const char *buf, int sz) {
        if (sz < (int)(sizeof(type) + sizeof(length))) return -1;
    ...
    void print(){
        cout<<"type    : "<<static_cast<underlying_type_t<msg_type>>(type)<<"\n";
        cout<<"length  : "<<length<<"\n";
        cout<<"content : "<<message<<"\n";
    }
};
// ---- Handshake helpers ----
...
inline int client_handshake(int sockfd) {
    message msg;
    msg.set(msg_type::TYPE_1, "", 0);
    if (send_message(sockfd, msg) < 0) return -1;

    if (recv_message(sockfd, msg) < 0) return -1;
    if (msg.type != msg_type::TYPE_2) return -2;
    //switch to udp
    cout<<"port recieved : "<<atoi(msg.message);
    return 0;
}
...
inline int server_handshake(int sockfd,const char* UDP_PORT) {
    message msg;
    if (recv_message(sockfd, msg) < 0) return -1;
    if (msg.type != msg_type::TYPE_1) return -2;

    msg.set(msg_type::TYPE_2, UDP_PORT, sizeof(UDP_PORT));
    if (send_message(sockfd, msg) < 0) return -1;
    //now switch to udp
