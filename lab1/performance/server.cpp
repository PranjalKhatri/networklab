...
void tcp_server(int port, size_t msg_size, size_t total_kb)
{
    ...
    if (bind(server_fd, (sockaddr *)&address, sizeof(address)) < 0) { perror("bind"); close(server_fd); return; }
    if (listen(server_fd, 1) < 0) { perror("listen"); close(server_fd); return; }

    std::cout << "[TCP] Waiting for connection on port " << port << "...\n";
    int sock = accept(server_fd, nullptr, nullptr);
    if (sock < 0) { perror("accept"); close(server_fd); return; }
    ...
    {
        MessageHeader hdr;
        if (recv_all(sock, (char*)&hdr, sizeof(hdr)) <= 0) break;
    ...
    while (sent < total_bytes)
    {
        ...
        if (send_all(sock, send_buffer.data(), sizeof(hdr) + msg_size) <= 0) break;
        sent += msg_size;
    }
    // send DONE
