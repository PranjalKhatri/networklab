...
void run_tcp(const char* server_ip, int port, size_t msg_size, size_t total_kb) {
    ...
    if (inet_pton(AF_INET, server_ip, &servaddr.sin_addr) <= 0) {
        perror("inet_pton"); close(sockfd); return;
    }
    ...
    while (sent < total_bytes) {
        ...
        if (send_all(sockfd, packet.data(), packet.size()) <= 0) break;
        sent += msg_size;
    }
    // Send DONE
    ...
    while (true) {
        MessageHeader hdr;
        if (recv_all(sockfd, (char*)&hdr, sizeof(hdr)) <= 0) break;
        if (hdr.payload_size == 0) break; // DONE
        std::vector<char> payload(hdr.payload_size);
        if (recv_all(sockfd, payload.data(), hdr.payload_size) <= 0) break;
        if (first_recv == 0) first_recv = now_ns();
        last_recv = now_ns();
